...
return t=t.map((t=>(p[t]&&(i+=t.length),p[t]||t))),i>=e?n(t):""})(t,e))))),_=(t,e=!0)=>{let i=t.length>1?1:0
...
i.initEvent(e,!0,!1),t.dispatchEvent(i)},q=(t,e)=>{Object.assign(t.style,e)},D=(t,...e)=>{var i=N(e);(t=R(t)).map((t=>{i.map((e=>{t.classList.add(e)}))}))},H=(t,...e)=>{var i=N(e);(t=R(t)).map((t=>{i.map((e=>{t.classList.remove(e)}))}))},N=e=>{var i=[]
...
t&&(p=t.textContent)}var h,g,f,v,m,y,O={placeholder:p,options:[],optgroups:[],items:[],maxItems:null}
return"select"===u?(g=O.options,f={},v=1,m=t=>{var e=Object.assign({},t.dataset),i=n&&e[n]
...
c[o]=c[o]||t.textContent,c[r]=c[r]||i,c[l]=c[l]||t.disabled,c[a]=c[a]||e,c.$option=t,f[i]=c,g.push(c)}t.selected&&O.items.push(i)}},O.maxItems=e.hasAttribute("multiple")?null:1,t.iterate(e.children,(e=>{var i,s,n
"optgroup"===(h=e.tagName.toLowerCase())?((n=m(i=e))[c]=n[c]||i.getAttribute("label")||"",n[d]=n[d]||v++,n[l]=n[l]||i.disabled,O.optgroups.push(n),s=n[d],t.iterate(i.children,(t=>{y(t,s)}))):"option"===h&&y(e)}))):(()=>{const i=e.getAttribute(n)
if(i)O.options=JSON.parse(i),t.iterate(O.options,(t=>{O.items.push(t[r])}))
else{var l=e.value.trim()||""
if(!s.allowEmptyOption&&!l.length)return
const i=l.split(s.delimiter)
t.iterate(i,(t=>{const e={}
e[o]=t,e[r]=t,O.options.push(e)})),O.items=i}})(),Object.assign({},U,O,i)}var at=0
...
e.replaceChild(t.firstChild,t),e.normalize()})),m.query.length&&m.tokens.length&&E(m.tokens,(t=>{G(b,t.regex)})))
